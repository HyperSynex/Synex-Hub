...
local GUI_NAME = "Synex-Hub _v1.0"
...
local function createLayout(parent, direction, padding)
    -- Default values if you don't provide them
    ...
    if parent:IsA("ScrollingFrame") then
        -- Use AutomaticCanvasSize if you're on a modern Roblox version (simplest way)
        -- But for custom math/buffer, we use this optimized listener:
        layout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
            task.defer(function() -- Defer avoids "depth" errors during UI updates
                local contentSize = layout.AbsoluteContentSize
                if direction == Enum.FillDirection.Vertical then
                    parent.CanvasSize = UDim2.new(0, 0, 0, contentSize.Y + 20) -- Reduced buffer to 20
                else
                    parent.CanvasSize = UDim2.new(0, contentSize.X + 20, 0, 0)
                end
            end)
        end)
    end
