...
local COUNTER_API = "https://api.counterapi.dev/v2/hypersynexs-team-3806/Synex-Hub" 
local FOLDER_NAME = "SynexRecords"
...
local function getGlobalExecutions()
    ...
    if success then
        ...
        if data.count then count = tostring(data.count)
        elseif data.value then count = tostring(data.value)
        else count = "Error" end
    else count = "N/A" end
    return count
end
...
local function getUserExecutionCount()
    ...
    if isfile(filePath) then
        pcall(function()
            ...
            if data and data.Executes then count = data.Executes end
        end)
    end
    count = count + 1
    pcall(function()
        local newData = { ["Executes"] = count, ["LastExecute"] = tostring(os.date("%x %X")) }
...
local function sendWebhook(globalCount, userCount)
    ...
    -- you can modify the code below in the comment to change what is sent to the webhook
    -- this was originally enabled :/
    --[[
    local data = {
        ["content"] = "",
        ["embeds"] = {{
            ["title"] = "🚀 Synex-Hub  Executed Sucessfully!",
            ...
            ["footer"] = {["text"] = "Synex Logger | " .. os.date("%X")}
        }}
    }
    ]]
    local jsonData = HttpService:JSONEncode(data)
    local httpRequest = (syn and syn.request) or (http and http.request) or http_request or (fluxus and fluxus.request) or request
    -- uncomment this for sending to the webhook
    --if httpRequest then pcall(function() httpRequest({ Url = EXECUTION_WEBHOOK, Method = "POST", Headers = {["Content-Type"] = "application/json"}, Body = jsonData }) end) end
end

local function sendFeedback(msg)
    -- im leaving this enabled
    ...
    local data = {
        ...
        ["embeds"] = {{
            ...
            ["footer"] = {["text"] = "Synex Feedback System | " .. os.date("%X")}
        }}
    }
    local jsonData = HttpService:JSONEncode(data)
    local httpRequest = (syn and syn.request) or (http and http.request) or http_request or (fluxus and fluxus.request) or request
    if httpRequest then pcall(function() httpRequest({ Url = FEEDBACK_WEBHOOK, Method = "POST", Headers = {["Content-Type"] = "application/json"}, Body = jsonData }) end) end
end
...
if _G.OriginalMaxZoom == nil then _G.OriginalMaxZoom = Player.CameraMaxZoomDistance end
...
Settings.QuickSitPos = nil
Settings.CurrentTheme = "Cyan"

local Themes = {
   {
    Cyan = { MainBg = Color3.fromRGB(15, 15, 15), SidebarBg = Color3.fromRGB(20, 20, 22), ContentBg = Color3.fromRGB(25, 25, 28), Accent = Color3.fromRGB(0, 170, 255), TextPrimary = Color3.fromRGB(255, 255, 255), TextSecondary = Color3.fromRGB(160, 165, 170), Outline = Color3.fromRGB(45, 45, 50), ItemHover = Color3.fromRGB(35, 35, 40), Red = Color3.fromRGB(255, 80, 80), Green = Color3.fromRGB(80, 255, 130), Yellow = Color3.fromRGB(255, 210, 60) },
    Red = { MainBg = Color3.fromRGB(15, 12, 12), SidebarBg = Color3.fromRGB(22, 18, 18), ContentBg = Color3.fromRGB(28, 24, 24), Accent = Color3.fromRGB(255, 50, 60), TextPrimary = Color3.fromRGB(255, 250, 250), TextSecondary = Color3.fromRGB(180, 160, 160), Outline = Color3.fromRGB(50, 40, 40), ItemHover = Color3.fromRGB(40, 32, 32), Red = Color3.fromRGB(255, 75, 75), Green = Color3.fromRGB(75, 255, 120), Yellow = Color3.fromRGB(255, 200, 50) },
    Green = { MainBg = Color3.fromRGB(12, 15, 12), SidebarBg = Color3.fromRGB(18, 22, 18), ContentBg = Color3.fromRGB(24, 28, 24), Accent = Color3.fromRGB(50, 255, 120), TextPrimary = Color3.fromRGB(250, 255, 250), TextSecondary = Color3.fromRGB(160, 180, 160), Outline = Color3.fromRGB(40, 50, 40), ItemHover = Color3.fromRGB(32, 40, 32), Red = Color3.fromRGB(255, 75, 75), Green = Color3.fromRGB(75, 255, 120), Yellow = Color3.fromRGB(255, 200, 50) },
    Purple = { MainBg = Color3.fromRGB(15, 12, 18), SidebarBg = Color3.fromRGB(22, 18, 25), ContentBg = Color3.fromRGB(28, 24, 32), Accent = Color3.fromRGB(180, 80, 255), TextPrimary = Color3.fromRGB(250, 250, 255), TextSecondary = Color3.fromRGB(170, 160, 185), Outline = Color3.fromRGB(50, 45, 60), ItemHover = Color3.fromRGB(40, 35, 50), Red = Color3.fromRGB(255, 75, 75), Green = Color3.fromRGB(75, 255, 120), Yellow = Color3.fromRGB(255, 200, 50) },
    Orange = { MainBg = Color3.fromRGB(18, 15, 12), SidebarBg = Color3.fromRGB(25, 21, 18), ContentBg = Color3.fromRGB(32, 28, 24), Accent = Color3.fromRGB(255, 145, 50), TextPrimary = Color3.fromRGB(255, 252, 250), TextSecondary = Color3.fromRGB(185, 170, 160), Outline = Color3.fromRGB(60, 50, 45), ItemHover = Color3.fromRGB(50, 40, 35), Red = Color3.fromRGB(255, 75, 75), Green = Color3.fromRGB(75, 255, 120), Yellow = Color3.fromRGB(255, 200, 50) },
    Midnight = { MainBg = Color3.fromRGB(8, 8, 12), SidebarBg = Color3.fromRGB(12, 12, 18), ContentBg = Color3.fromRGB(16, 16, 24), Accent = Color3.fromRGB(80, 120, 255), TextPrimary = Color3.fromRGB(230, 230, 255), TextSecondary = Color3.fromRGB(140, 140, 180), Outline = Color3.fromRGB(35, 35, 55), ItemHover = Color3.fromRGB(30, 30, 45), Red = Color3.fromRGB(255, 80, 80), Green = Color3.fromRGB(80, 255, 150), Yellow = Color3.fromRGB(255, 220, 100) },
    Synapse = { MainBg = Color3.fromRGB(35, 35, 35), SidebarBg = Color3.fromRGB(45, 45, 45), ContentBg = Color3.fromRGB(40, 40, 40), Accent = Color3.fromRGB(255, 255, 255), TextPrimary = Color3.fromRGB(255, 255, 255), TextSecondary = Color3.fromRGB(180, 180, 180), Outline = Color3.fromRGB(65, 65, 65), ItemHover = Color3.fromRGB(55, 55, 55), Red = Color3.fromRGB(255, 100, 100), Green = Color3.fromRGB(100, 255, 100), Yellow = Color3.fromRGB(255, 255, 100) },
    Gold = { MainBg = Color3.fromRGB(20, 18, 15), SidebarBg = Color3.fromRGB(28, 25, 20), ContentBg = Color3.fromRGB(35, 32, 28), Accent = Color3.fromRGB(255, 190, 50), TextPrimary = Color3.fromRGB(255, 255, 230), TextSecondary = Color3.fromRGB(180, 170, 140), Outline = Color3.fromRGB(60, 55, 45), ItemHover = Color3.fromRGB(50, 45, 35), Red = Color3.fromRGB(255, 80, 80), Green = Color3.fromRGB(100, 255, 100), Yellow = Color3.fromRGB(255, 190, 50) },
    Toxic = { MainBg = Color3.fromRGB(8, 12, 8), SidebarBg = Color3.fromRGB(12, 18, 12), ContentBg = Color3.fromRGB(16, 24, 16), Accent = Color3.fromRGB(150, 255, 0), TextPrimary = Color3.fromRGB(220, 255, 220), TextSecondary = Color3.fromRGB(130, 160, 130), Outline = Color3.fromRGB(35, 55, 35), ItemHover = Color3.fromRGB(25, 45, 25), Red = Color3.fromRGB(255, 70, 70), Green = Color3.fromRGB(70, 255, 70), Yellow = Color3.fromRGB(230, 230, 0) },
    CottonCandy = { MainBg = Color3.fromRGB(255, 245, 250), SidebarBg = Color3.fromRGB(245, 230, 240), ContentBg = Color3.fromRGB(255, 255, 255), Accent = Color3.fromRGB(255, 150, 200), TextPrimary = Color3.fromRGB(100, 80, 100), TextSecondary = Color3.fromRGB(150, 130, 150), Outline = Color3.fromRGB(230, 210, 225), ItemHover = Color3.fromRGB(240, 220, 235), Red = Color3.fromRGB(255, 120, 120), Green = Color3.fromRGB(120, 230, 160), Yellow = Color3.fromRGB(255, 220, 140) },
    Ocean = { MainBg = Color3.fromRGB(8, 15, 25), SidebarBg = Color3.fromRGB(12, 25, 40), ContentBg = Color3.fromRGB(18, 35, 55), Accent = Color3.fromRGB(0, 210, 255), TextPrimary = Color3.fromRGB(230, 245, 255), TextSecondary = Color3.fromRGB(140, 170, 200), Outline = Color3.fromRGB(40, 65, 95), ItemHover = Color3.fromRGB(30, 55, 85), Red = Color3.fromRGB(255, 100, 100), Green = Color3.fromRGB(100, 255, 200), Yellow = Color3.fromRGB(255, 230, 120) },
    Vaporwave = { MainBg = Color3.fromRGB(25, 15, 40), SidebarBg = Color3.fromRGB(35, 20, 55), ContentBg = Color3.fromRGB(45, 25, 70), Accent = Color3.fromRGB(255, 100, 255), TextPrimary = Color3.fromRGB(100, 255, 255), TextSecondary = Color3.fromRGB(200, 150, 255), Outline = Color3.fromRGB(80, 50, 120), ItemHover = Color3.fromRGB(65, 40, 100), Red = Color3.fromRGB(255, 80, 150), Green = Color3.fromRGB(80, 255, 180), Yellow = Color3.fromRGB(255, 255, 150) },
    Dracula = { MainBg = Color3.fromRGB(33, 34, 44), SidebarBg = Color3.fromRGB(40, 42, 54), ContentBg = Color3.fromRGB(50, 52, 64), Accent = Color3.fromRGB(189, 147, 249), TextPrimary = Color3.fromRGB(248, 248, 242), TextSecondary = Color3.fromRGB(139, 151, 190), Outline = Color3.fromRGB(68, 71, 90), ItemHover = Color3.fromRGB(55, 55, 75), Red = Color3.fromRGB(255, 85, 85), Green = Color3.fromRGB(80, 250, 123), Yellow = Color3.fromRGB(241, 250, 140) }
}
...
local Theme = Themes[Settings.CurrentTheme] or Themes.Midnight
local GUI_NAME = "Synex-Hub Premium"
local NotificationLayout

--// 🧊 BACKGROUND BLUR (paid feel)
local Blur = Instance.new("BlurEffect")
Blur.Size = 14
Blur.Parent = game:GetService("Lighting")

--// ✨ PREMIUM STROKE (soft glow)
...
local function SendNotification(text, color)
    ...
    content.Position = UDim2.new(1, 40, 0, 0)
    content.BackgroundColor3 = Theme.SidebarBg
    content.BackgroundTransparency = 0.2
    content.Parent = container

    Instance.new("UICorner", content).CornerRadius = UDim.new(0, 10)
    AddStroke(content, color or Theme.Accent, 1.5)
    AddGlow(content, color or Theme.Accent)
...
--// ⚡ PREMIUM LAYOUT
local function createLayout(parent, direction, padding)
    ...
    if parent:IsA("ScrollingFrame") then
        layout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
            task.defer(function()
                ...
                if direction == Enum.FillDirection.Vertical then
                    parent.CanvasSize = UDim2.new(0, 0, 0, size.Y + 30)
                else
                    parent.CanvasSize = UDim2.new(0, size.X + 30, 0, 0)
                end
            end)
        end)
    end
...
local function createToggleSwitch(parent, text, description, callback, externalControl, noSave)
    ...
    label.Position = UDim2.new(0.05, 0, 0.15, 0)
    label.Text = text
    label.TextColor3 = Theme.TextPrimary
    label.TextXAlignment = Enum.TextXAlignment.Left
...
local function createSlider(parent, text, description, minVal, maxVal, defaultVal, callback)
    local container = Instance.new("Frame")
    container.Size = UDim2.new(0.96, 0, 0, 60)
    container.BackgroundColor3 = Theme.ContentBg
    container.Parent = parent
    Instance.new("UICorner", container).CornerRadius = UDim.new(0, 6)
    AddStroke(container, Theme.Outline, 1)
    ...
    sliderBg.Position = UDim2.new(0.04, 0, 0.8, 0)
...
local function createTextBox(parent, placeholder, description, callback)
    ...
    local descLabel = Instance.new("TextLabel")
    descLabel.Size = UDim2.new(0.95, 0, 0, 20)
    descLabel.Position = UDim2.new(0.025, 0, 0.05, 0)
    descLabel.BackgroundTransparency = 1
    descLabel.Text = description or ""
    descLabel.TextColor3 = Theme.TextSecondary
    descLabel.Font = Enum.Font.Gotham
    ...
    box.Position = UDim2.new(0.025, 0, 0.45, 0)
...
local function createConfirmation(parent, text, onYes)
    ...
    yesBtn.MouseButton1Click:Connect(function() onYes() screen:Destroy() end)
    noBtn.MouseButton1Click:Connect(function() screen:Destroy() end)
end

local oldScript = _G.Synex_Cleanup
...
local function Cleanup()
    ...
    pcall(function()
        ...
        Player.CameraMaxZoomDistance = _G.OriginalMaxZoom or 128
        if Player.Character then
            ...
            if root then
                for _, v in pairs(root:GetChildren()) do
                    if v.Name == "Synex_BodyGyro" or v.Name == "Synex_BodyVel" or v.Name == "Synex_Spin" or v.Name == "Synex_Fling" then v:Destroy() end
                end
            end
...
local function BuildInterface(isReload)
    ...
    local defaultMaxZoom = _G.OriginalMaxZoom or Player.CameraMaxZoomDistance
    ...
    local QuickSitBtn = Instance.new("TextButton")
    QuickSitBtn.Name = "QuickSitButton"
    QuickSitBtn.Size = UDim2.new(0, 50, 0, 50)
    QuickSitBtn.Position = Settings.QuickSitPos and UDim2.new(Settings.QuickSitPos.X, Settings.QuickSitPos.XOff, Settings.QuickSitPos.Y, Settings.QuickSitPos.YOff) or UDim2.new(0.01, 0, 0.75, 0)
    QuickSitBtn.BackgroundColor3 = Theme.SidebarBg
    QuickSitBtn.Text = "SIT" 
    QuickSitBtn.TextColor3 = Theme.Accent
    QuickSitBtn.Font = Enum.Font.GothamBlack
    QuickSitBtn.TextSize = 16
    QuickSitBtn.Visible = false
    QuickSitBtn.Draggable = true
    QuickSitBtn.Parent = ScreenGui
    Instance.new("UICorner", QuickSitBtn).CornerRadius = UDim.new(0, 14)
    local QuickSitStroke = AddStroke(QuickSitBtn, Theme.Accent, 2)

    QuickSitBtn.MouseButton1Click:Connect(function()
    ...
    local function saveBtnPos()
        ...
        Settings.QuickSitPos = {X = QuickSitBtn.Position.X.Scale, XOff = QuickSitBtn.Position.X.Offset, Y = QuickSitBtn.Position.Y.Scale, YOff = QuickSitBtn.Position.Y.Offset}
        saveSettings()
    end
    AddConnection(UserInputService.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then saveBtnPos() end
    end))
    ...
    SideBar.InputChanged:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then dragInput = input end end)
    ...
    local CombatFrame = CreatePage("Combat")
    ...
    local function SwitchTab(btn, frame)
        ...
        for _, child in pairs(TabButtonsContainer:GetChildren()) do
            if child:IsA("TextButton") then
                ...
                if child:FindFirstChild("Bar") then TweenService:Create(child.Bar, TweenInfo.new(0.2), {Size = UDim2.new(0, 0, 0.8, 0)}):Play() end
            end
        end
        TweenService:Create(btn, TweenInfo.new(0.2), {BackgroundColor3 = Theme.ContentBg, TextColor3 = Theme.TextPrimary}):Play()
        if btn:FindFirstChild("Bar") then TweenService:Create(btn.Bar, TweenInfo.new(0.2), {Size = UDim2.new(0, 3, 0.8, 0)}):Play() end
        currentTab = btn
    end
    ...
    local TabPVP = CreateTabButton("⚔️ Combat", CombatFrame)
    ...
    addChangeLogItem("1. Added descriptions to all features")
    addChangeLogItem("2. Fixed Fling 'Invisible Wall' Bug")
    addChangeLogItem("3. Improved Fling Noclip")
    addChangeLogItem("3. This Version Is Only For Developer (CorTexZer)")

    local flinging = false
    local Noclipping = nil
    
    local function FlingNoclipLoop()
    ...
    local function toggleFling(state)
        ...
        if state then
            if char then
                ...
                Noclipping = RunService.Stepped:Connect(FlingNoclipLoop)
                local root = char:FindFirstChild("HumanoidRootPart")
                if root then
                    local bambam = Instance.new("BodyAngularVelocity")
                    bambam.Name = "Synex_Fling"
                    bambam.Parent = root
                    bambam.AngularVelocity = Vector3.new(0, 99999, 0)
                    bambam.MaxTorque = Vector3.new(0, math.huge, 0)
                    bambam.P = math.huge
                    task.spawn(function()
                        while flinging and root and root.Parent do
                            bambam.AngularVelocity = Vector3.new(0, 99999, 0)
                            task.wait(0.2)
            ...
            if char then
                local root = char:FindFirstChild("HumanoidRootPart")
                if root and root:FindFirstChild("Synex_Fling") then root.Synex_Fling:Destroy() end
    ...
    createToggleSwitch(CombatFrame, "🎯 Enable Hitbox Expander", "Enlarges player hitboxes for easier aiming.", function(state)
        hitboxEnabled = state
        if not state then
            for _, v in pairs(game.Players:GetPlayers()) do
                if v ~= Player and v.Character and v.Character:FindFirstChild("HumanoidRootPart") then
    ...
    createSlider(CombatFrame, "📏 Hitbox Size", "Adjust the radius of the hitbox expansion.", 2, 50, 10, function(val) hitboxSize = val end)
    ...
    local function stopFly()
        ...
        if Player.Character then
            local root = Player.Character:FindFirstChild("HumanoidRootPart")
            if root then
                if root:FindFirstChild("Synex_BodyGyro") then root.Synex_BodyGyro:Destroy() end
                if root:FindFirstChild("Synex_BodyVel") then root.Synex_BodyVel:Destroy() end
            end
            local hum = Player.Character:FindFirstChild("Humanoid")
            if hum then
                hum.PlatformStand = false
    ...
    local function startFly()
        ...
        flyBodyGyro.Name = "Synex_BodyGyro"
        flyBodyGyro.P = 9e4
        flyBodyGyro.maxTorque = Vector3.new(9e9, 9e9, 9e9)
        flyBodyGyro.CFrame = root.CFrame
        flyBodyGyro.Parent = root
        flyBodyVel = Instance.new("BodyVelocity")
        flyBodyVel.Name = "Synex_BodyVel"
        ...
        task.spawn(function()
            while flyEnabled and char and hum.Parent and hum.Health > 0 do
                ...
                if moveDir.Magnitude > 0 then
                    ...
                    if newVel.Magnitude > 0 then newVel = newVel.Unit end
    ...
    local function onCharacterAdded(char)
        ...
        AddConnection(humanoid.Died:Connect(function() if flyEnabled then toggleFlyFunc(false) end end))
    end
    if Player.Character then onCharacterAdded(Player.Character) end
    AddConnection(Player.CharacterAdded:Connect(onCharacterAdded))

    MenuFlySwitch = createToggleSwitch(MovementFrame, "🛸 Enable Fly", "Toggle flight mode to move freely in the air.", function(state)
        if state ~= flyEnabled then toggleFlyFunc(state) end
    end, true, false)

    createSlider(MovementFrame, "⚡ Flight Speed", "Control how fast you fly.", 1, 50, 3, function(val)
        flySpeed = val
        Settings.FlightSpeed = val
    end)

    createToggleSwitch(MovementFrame, "🔲 Show Quick Fly Button", "Toggle the visibility of the HUD fly button.", function(state)
    ...
    createToggleSwitch(MovementFrame, "👟 Speed Boost", "Enable faster walking speed.", function(state) speedEnabled = state end)
    createSlider(MovementFrame, "💨 Speed Value", "Adjust your walking speed multiplier.", 16, 250, 16, function(val) currentSpeed = val end)

    createToggleSwitch(MovementFrame, "🦗 Jump Boost", "Enable higher jumping power.", function(state) jumpEnabled = state end)
    createSlider(MovementFrame, "⏫ Jump Value", "Adjust your jump height multiplier.", 16, 250, 20, function(val) currentJump = val end)

    AddConnection(RunService.Stepped:Connect(function()
        if Player.Character and Player.Character:FindFirstChild("Humanoid") then
            local hum = Player.Character.Humanoid
            if speedEnabled then hum.WalkSpeed = currentSpeed else if hum.WalkSpeed ~= _G.DefaultWalkSpeed then hum.WalkSpeed = _G.DefaultWalkSpeed end end
            if jumpEnabled then hum.UseJumpPower = true hum.JumpPower = currentJump * 5 else if hum.JumpPower ~= _G.DefaultJumpPower then hum.JumpPower = _G.DefaultJumpPower end end
        end
    end))
    ...
    local function toggleNoclipFunc(state)
        ...
        if state then NoclipConnection = RunService.Stepped:Connect(NoclipLoop) else if NoclipConnection then NoclipConnection:Disconnect() end if Player.Character then local hum = Player.Character:FindFirstChild("Humanoid") if hum then hum:ChangeState(Enum.HumanoidStateType.GettingUp) end end end
    end

    QuickNoclipBtn.MouseButton1Click:Connect(function() toggleNoclipFunc(not noclipEnabled) end)

    MenuNoclipSwitch = createToggleSwitch(MovementFrame, "👻 Noclip (Ghost Mode)", "Walk through walls and obstacles.", function(state)
    ...
    createButton(MovementFrame, "🪄 TP Tool", "Equips a tool to teleport where you click.", Theme.Purple, function()
        local toolName = "Teleport Tool"
        ...
        tool.Activated:Connect(function()
            ...
            if root and mouse and mouse.Hit then
                ...
                for _, v in ipairs(char:GetDescendants()) do if v:IsA("BasePart") then v.Velocity = V3 v.RotVelocity = V3 end end
            end
        end)
        SendNotification("TP Tool Added!", Theme.Green)
    end)

    AddConnection(UserInputService.JumpRequest:Connect(function() if _G.InfJump and Player.Character then Player.Character:FindFirstChildOfClass('Humanoid'):ChangeState("Jumping") end end))
    ...
    createButton(PositionsFrame, "💾 Save Current Position", "Save your current coordinates to the list.", Theme.Green, function()
        if posNameBox.Text ~= "" then
            ...
            if root then
                local positions = getGamePositions()
                local newPos = { Name = posNameBox.Text, X = root.Position.X, Y = root.Position.Y, Z = root.Position.Z }
    ...
    local posDropdownBtn = createButton(PositionsFrame, "👇 Select Position", "Open the list of saved positions.", Theme.Outline, function() 
        refreshPositionList()
        posDropdownFrame.Visible = not posDropdownFrame.Visible
    end)

    createButton(PositionsFrame, "🌀 Teleport to Position", "Instantly teleport to the selected location.", Theme.Accent, function()
        ...
        else SendNotification("Select a position first!", Theme.Red) end
    end)

    createButton(PositionsFrame, "🦅 Tween to Position", "Fly smoothly to the selected location.", Theme.Yellow, function()
        if selectedPos and Player.Character and Player.Character:FindFirstChild("HumanoidRootPart") then
            ...
            workspace.Gravity = 0
            tween:Play()
            tween.Completed:Connect(function() workspace.Gravity = oldGrav end)
        else SendNotification("Select a position first!", Theme.Red) end
    end)

    createButton(PositionsFrame, "🚶 Walk to Position", "Pathfind/Walk to the selected location.", Theme.Purple, function()
    ...
    createButton(PositionsFrame, "🗑️ Delete Position", "Remove the selected position from the list.", Theme.Red, function()
        if selectedPos then
            local positions = getGamePositions()
            for i, pos in ipairs(positions) do if pos == selectedPos then table.remove(positions, i) break end end
            PositionsData[tostring(game.PlaceId)] = positions
            savePositions()
            selectedPos = nil
            posLabel.Text = "Selected: None"
    ...
    local function createESP(player)
        ...
        local function updater()
            local char = player.Character
            if not char or not espEnabled then folder:ClearAllChildren() return end
            local root = char:FindFirstChild("HumanoidRootPart")
            if root then
    ...
    createToggleSwitch(VisualsFrame, "🔍 Player ESP", "Highlight players through walls.", function(state)
        espEnabled = state
        if state then for _, p in pairs(game.Players:GetPlayers()) do createESP(p) end else for _, child in pairs(ScreenGui:GetChildren()) do if child:IsA("Folder") and string.find(child.Name, "_ESP") then child:Destroy() end end end
    end)
    ...
    AddConnection(RunService.Heartbeat:Connect(function()
        if noFogEnabled then
            ...
            for _, v in pairs(Lighting:GetChildren()) do if v:IsA("Atmosphere") or v:IsA("Clouds") then v:Destroy() end end
        end
    end))

    createToggleSwitch(VisualsFrame, "🌫️ No Fog", "Removes fog and clouds for clear vision.", function(state)
        noFogEnabled = state
        if not state then Lighting.FogStart = _G.OriginalFogStart or 0 Lighting.FogEnd = _G.OriginalFogEnd or 8000 end
    end)

    local fullbrightEnabled = false
    createToggleSwitch(VisualsFrame, "☀️ Fullbright (Light)", "Max out brightness to see in the dark.", function(state)
        fullbrightEnabled = state
        if state then Lighting.Ambient = Color3.new(1, 1, 1) Lighting.Brightness = 2 Lighting.ClockTime = 14 else Lighting.Ambient = _G.OriginalAmbient Lighting.Brightness = _G.OriginalBrightness Lighting.ClockTime = _G.OriginalClockTime end
    end)
    ...
    local function TurnVisible()
        ...
        pcall(function()
            ...
            if InvisibleCharacter and InvisibleCharacter.Parent then
                ...
                if root then storedCF = root.CFrame end
            ...
            if realChar and realChar.Parent then
                ...
                if root then
                    root.CFrame = storedCF or root.CFrame
    ...
    local function TurnInvisible()
        ...
        InvisibleCharacter = realChar:Clone()
        InvisibleCharacter.Name = "Ghost_"..Player.Name
        ...
        local cf = realRoot.CFrame
        realRoot.CFrame = CFrame.new(0, 100000, 0)
        InvisibleCharacter.Parent = workspace
        InvisibleCharacter:FindFirstChild("HumanoidRootPart").CFrame = cf
        ...
        invisFix = RunService.Stepped:Connect(function()
            pcall(function()
                if realChar and realChar.Parent and realRoot then
                    realRoot.CFrame = CFrame.new(0, 100000, 0)
                    realRoot.Velocity = Vector3.new(0,0,0)
                end
            end)
        end)
        local cloneHum = InvisibleCharacter:FindFirstChild("Humanoid")
        if cloneHum then invisDied = cloneHum.Died:Connect(function() TurnVisible() RespawnReal() end) end
    end

    createToggleSwitch(VisualsFrame, "👻 Invisible Mode (FE)", "Makes your character invisible to others.", function(state)
        if state then TurnInvisible() else TurnVisible() end
    end)

    createSlider(VisualsFrame, "🔭 Field Of View (FOV)", "Adjust the camera field of view.", 70, 120, 70, function(val) workspace.CurrentCamera.FieldOfView = val end)

    local zoomToggleState = false
    local customZoomValue = 128
    
    createToggleSwitch(VisualsFrame, "🔭 Enable Custom Zoom", "Override the maximum camera zoom distance.", function(state)
        zoomToggleState = state
        if state then Player.CameraMaxZoomDistance = customZoomValue else Player.CameraMaxZoomDistance = defaultMaxZoom end
    end)

    createSlider(VisualsFrame, "📏 Zoom Distance Value", "Set the max distance for custom zoom.", 50, 10000, 128, function(val)
        customZoomValue = val
        if zoomToggleState then Player.CameraMaxZoomDistance = val end
    end)

    createButton(VisualsFrame, "♾️ Unlimited Zoom", "Allows the camera to zoom out infinitely.", Theme.Accent, function()
        Player.CameraMaxZoomDistance = 9e9
        SendNotification("Max Zoom set to Unlimited!", Theme.Green)
    end)
    ...
    local function updateMimic()
        ...
        if mimicEnabled and selectedPlayer then
            mimicConnection = selectedPlayer.Chatted:Connect(function(msg)
                ...
                if events and events:FindFirstChild("SayMessageRequest") then events.SayMessageRequest:FireServer(unpack(args)) else local tcs = game:GetService("TextChatService") if tcs and tcs:FindFirstChild("TextChannels") and tcs.TextChannels:FindFirstChild("RBXGeneral") then tcs.TextChannels.RBXGeneral:SendAsync(msg) end end
            end)
        end
    end
    ...
    local dropdownBtn = createButton(PlayersFrame, "👇 Select Player", "Open the list of players to select one.", Theme.Outline, function() end)
    ...
    createToggleSwitch(PlayersFrame, "🦜 Chat Mimic", "Repeat whatever the selected player types.", function(state)
        mimicEnabled = state
        updateMimic()
    end)

    createButton(PlayersFrame, "🌀 Teleport to Player", "Instantly teleport to the selected player.", Theme.Accent, function()
        if selectedPlayer and selectedPlayer.Character and selectedPlayer.Character:FindFirstChild("HumanoidRootPart") then
            Player.Character:SetPrimaryPartCFrame(selectedPlayer.Character.HumanoidRootPart.CFrame)
        end
    end)

    createButton(PlayersFrame, "🧲 Bring Player (Client)", "Visually bring the player to you (Local Only).", Theme.Green, function()
        if selectedPlayer and selectedPlayer.Character and selectedPlayer.Character:FindFirstChild("HumanoidRootPart") then
    ...
    createToggleSwitch(PlayersFrame, "👀 Spectate Player", "Watch the selected player's perspective.", function(state)
        spectating = state
        local cam = workspace.CurrentCamera
        if not state then cam.CameraSubject = Player.Character:FindFirstChild("Humanoid") end
    end)
    ...
    local mouse = Player:GetMouse()
    createToggleSwitch(PlayersFrame, "🖱️ Ctrl + Click TP", "Teleport to mouse cursor by holding Ctrl.", function(state)
    ...
    AddConnection(mouse.Button1Down:Connect(function()
        if clickTpEnabled and UserInputService:IsKeyDown(Enum.KeyCode.LeftControl) then
            ...
            if Player.Character then Player.Character:MoveTo(pos) end
        end
    end))
    ...
    afkScreenToggleBtn = createToggleSwitch(AFKFrame, "🌑 AFK Screen Mode", "Turns the screen black to save resources.", function(state)
        BlackScreen.Visible = state
        MainFrame.Visible = state
    end)
    BlackScreen.ZIndex = 5000
    CloseX.ZIndex = 5001
    CloseX.MouseButton1Click:Connect(function() afkScreenToggleBtn:SetState(false) end)

    local autoReconnectEnabled = false
    createToggleSwitch(AFKFrame, "🔄 Auto Reconnect", "Automatically rejoins if you get disconnected.", function(state)
        autoReconnectEnabled = state
    end)
    AddConnection(GuiService.ErrorMessageChanged:Connect(function()
        if autoReconnectEnabled then task.wait(5) TeleportService:Teleport(game.PlaceId, Player) end
    end))

    local antiAfkOn = false
    createToggleSwitch(AFKFrame, "🛡️ Anti-Kick (20m)", "Prevents Roblox from kicking you for inactivity.", function(state)
        antiAfkOn = state
        if state then for _, c in pairs(getconnections(Player.Idled)) do c:Disable() end else for _, c in pairs(getconnections(Player.Idled)) do c:Enable() end end
    end)
    AddConnection(Player.Idled:Connect(function()
        if antiAfkOn then VirtualUser:CaptureController() VirtualUser:ClickButton2(Vector2.new()) end
    end))

    local autoClickOn = false
    createToggleSwitch(AFKFrame, "👆 Auto Clicker", "Automatically clicks the mouse rapidly.", function(state)
        autoClickOn = state
        task.spawn(function()
            while autoClickOn do VirtualUser:CaptureController() VirtualUser:ClickButton1(Vector2.new()) task.wait(0.1) end
        end)
    end)

    local spinOn = false
    local spinVelocity
    createToggleSwitch(AFKFrame, "😵 Spin Bot (Anti-Kick)", "Spins your character to prevent AFK kicks.", function(state)
        spinOn = state
        if state then
            local root = Player.Character and Player.Character:FindFirstChild("HumanoidRootPart")
            if root then
                spinVelocity = Instance.new("BodyAngularVelocity")
                spinVelocity.Name = "Synex_Spin"
            ...
            if Player.Character and Player.Character:FindFirstChild("HumanoidRootPart") then
                if Player.Character.HumanoidRootPart:FindFirstChild("Synex_Spin") then
                    Player.Character.HumanoidRootPart.Synex_Spin:Destroy()
                end
            end
        end
    end)
    ...
    createToggleSwitch(AFKFrame, "🐇 Auto Jump", "Automatically makes your character jump.", function(state)
        autoJumpOn = state
        task.spawn(function()
            while autoJumpOn do
                if Player.Character and Player.Character:FindFirstChild("Humanoid") then
    ...
    createButton(ServerFrame, "🔁 Rejoin Server", "Reconnect to the same server instantly.", Theme.Yellow, function()
        saveSettings()
        TeleportService:Teleport(game.PlaceId, Player)
    end)

    createButton(ServerFrame, "⏭️ Server Hop (Low Pop)", "Join a different server with fewer players.", Theme.Green, function()
        saveSettings()
        ...
        until Server
        if Server.playing < Server.maxPlayers then TPS:TeleportToPlaceInstance(_place, Server.id, Player) else TPS:Teleport(game.PlaceId, Player) end
    end)
    ...
    local feedbackBox = createTextBox(SettingsFrame, "✍️ Write idea here...", "Send your suggestions to the dev.", function() end)
    
    createButton(SettingsFrame, "📨 Send Feedback", "Submit the written feedback.", Theme.Green, function()
        if feedbackBox.Text ~= "" then
            sendFeedback(feedbackBox.Text)
            feedbackBox.Text = ""
            SendNotification("Feedback Sent!", Theme.Green)
        else SendNotification("Cannot send empty feedback!", Theme.Red) end
    end)

    createButton(InfoFrame, "📋 Copy Job ID", "Copy server Job ID to clipboard.", Theme.Accent, function() setclipboard(game.JobId) end)

    local jobIdBox = createTextBox(InfoFrame, "🎫 Enter Job ID to Join...", "Paste Job ID here to join server.", function() end)
    createButton(InfoFrame, "🚀 Join by Job ID", "Teleport to the specified server.", Theme.Green, function()
        local jId = jobIdBox.Text
        if jId and jId ~= "" then TeleportService:TeleportToPlaceInstance(game.PlaceId, jId, Player) end
    end)

    task.spawn(function()
        local success, info = pcall(function() return MarketplaceService:GetProductInfo(game.PlaceId) end)
        if success and info then gameNameLbl.Text = info.Name else gameNameLbl.Text = "Unknown" end
    end)
    ...
    createToggleSwitch(SettingsFrame, "🔒 Lock GUI Drag", "Prevent the HUD buttons from moving.", function(state)
        ToggleBtn.Draggable = not state
        QuickFlyBtn.Draggable = not state
        QuickNoclipBtn.Draggable = not state
        QuickSitBtn.Draggable = not state
    end)
    
    createButton(SettingsFrame, "🔄 Reload Script", "Restart the script fresh.", Theme.Yellow, function()
        Cleanup()
        task.wait(0.1)
        BuildInterface(true)
    end)
    ...
    createButton(SettingsFrame, "🗑️ Reset Config", "Delete saved settings and reset.", Theme.Red, function()
    ...
    createButton(SettingsFrame, "⌨️ Change Keybind", "Cycle through available menu keys.", Theme.Accent, function()
    ...
    AddConnection(UserInputService.InputBegan:Connect(function(input, gpe)
        ...
        if input.KeyCode == Enum.KeyCode[Settings.MenuKey] then MainFrame.Visible = not MainFrame.Visible end
    end))

    ToggleBtn.MouseButton1Click:Connect(function() MainFrame.Visible = not MainFrame.Visible end)

    local themeDropdownBtn = createButton(SettingsFrame, "🎨 Theme: " .. Settings.CurrentTheme, "Change the UI color scheme.", Theme.Outline, function() end)
    ...
    if isReload then SendNotification("Nova Script Reloaded Successfully", Theme.Green) else SendNotification("Nova Script Loaded Successfully", Theme.Accent) end
end
